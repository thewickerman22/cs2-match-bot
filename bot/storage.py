...
class Storage:
    ...
    async def save_match_voice_returns(
        self,
        match_id: str,
        ...
        async with aiosqlite.connect(self.database_path) as db:
            await db.execute(
                "DELETE FROM match_voice_returns WHERE match_id = ?",
                (match_id,),
            )
            await db.executemany(
                """
                INSERT INTO match_voice_returns (match_id, discord_id, original_channel_id)
                VALUES (?, ?, ?)
                """,
                [
                    (match_id, discord_id, channel_id)
                    for discord_id, channel_id in original_channels.items()
                ],
            )
            await db.commit()

    async def get_match_voice_returns(self, match_id: str) -> dict[int, int]:
        async with aiosqlite.connect(self.database_path) as db:
            cursor = await db.execute(
                """
                SELECT discord_id, original_channel_id
                FROM match_voice_returns
                WHERE match_id = ?
                """,
                (match_id,),
            )
    ...
    async def clear_match_voice_returns(self, match_id: str) -> None:
        async with aiosqlite.connect(self.database_path) as db:
            await db.execute(
                "DELETE FROM match_voice_returns WHERE match_id = ?",
                (match_id,),

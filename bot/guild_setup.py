...
class GuildSetup:
    ...
    commands_player_message_id: int | None = None
    commands_admin_message_id: int | None = None
...
async def ensure_guild_setup(
    ...
    commands_channel = _resolve_text_channel(
        guild,
        category,
        COMMANDS_CHANNEL_NAME,
        existing.commands_channel_id if existing else None,
    )
    if commands_channel is None:
        commands_channel = await guild.create_text_channel(
            COMMANDS_CHANNEL_NAME,
            category=category,
            topic="Pinned command reference and admin quick actions (react on admin panel).",
            overwrites=read_only_overwrites,
        )
    ...
    return GuildSetup(
        ...
        commands_player_message_id=(
            existing.commands_player_message_id if existing else None
        ),
        commands_admin_message_id=(
            existing.commands_admin_message_id if existing else None
        ),
    )
